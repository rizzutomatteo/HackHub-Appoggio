...
| UC19 | Registrazione e Autenticazione | Visitatore      | Accesso    | Creator (crea Utente) + Controller              |
| UC20 | Consulta Hackathon             | Visitatore      | Accesso    | Information Expert (sola lettura)               |
| UC21 | Aggiungi Mentore               | Organizzatore   | Staff      | Creator (Incarico) — riuso `assegnaStaff`       |
...
3. **L'attore Visitatore non ha alcun caso d'uso.** `progetto.MD` (§Visitatore) lo descrive: "_accede liberamente al sito per consultare le informazioni pubbliche sugli hackathon. Non può accedere ad altre funzionalità finché non effettua la registrazione e l'accesso._" **UC19 Registrazione e Autenticazione** e **UC20 Consulta Hackathon** lo collegano al modello e saldano il **debito noto dell'autenticazione** (finora stub).
4. **Un requisito esplicito non modellato.** `progetto.MD` (§Organizzatore) dice che l'Organizzatore "_può aggiungere più Mentori all'hackathon anche successivamente alla sua creazione_". **UC21 Aggiungi Mentore** lo realizza (a costo minimo, riusando `assegnaStaff`/`Incarico`).
...
- **D4 — Autenticazione "sottile".** UC19 rende **concreto** il passaggio Visitatore → Utente (registrazione + accesso), finora trattato come pre-condizione stub (§4.1 delle iterazioni precedenti). **Non** si realizza uno strato di sicurezza completo (hashing robusto, sessioni, ruoli a runtime): si modella il minimo che collega l'attore Visitatore e dà un'identità reale all'"utente corrente". La sicurezza forte resta fuori scope (§4.5).
- **D5 — Scope dell'Organizzatore via `Incarico`.** In UC18 e UC21 l'Organizzatore vede/agisce **solo** sugli hackathon a cui è assegnato (Incarico, ruolo = Organizzatore) — stessa convenzione di scope dei Mentori (iter. 3, D3).
...
### UC19 — Registrazione e Autenticazione

| Campo                  | Valore                         |
| ---------------------- | ------------------------------ |
| **Nome**               | Registrazione e Autenticazione |
| **ID**                 | UC19                           |
| **Portata**            | HackHub (sistema)              |
| **Livello**            | Obiettivo utente               |
...
**Breve descrizione.** Un **Visitatore** crea un account fornendo i propri dati e **diventa Utente** registrato, ottenendo l'**accesso** alle funzionalità riservate. Realizza in modo **concreto** il passaggio Visitatore → Utente, finora trattato come pre-condizione stub.

**Evento scatenante.** Il Visitatore vuole partecipare (creare/iscrivere un team, ecc.) e deve prima registrarsi e accedere (`progetto.MD` §Visitatore).

**Parti interessate e interessi.**

- _Visitatore_: vuole ottenere un'identità nella piattaforma e poter accedere.
...
- Esiste un nuovo **`Utente`** registrato con le credenziali fornite.
- L'utente risulta **autenticato** (accesso effettuato).
...
5. Il sistema crea l'`Utente` con le credenziali.
6. Il sistema autentica l'utente (accesso effettuato) e conferma la registrazione.
...
**Autenticazione (accesso) di un utente già registrato.** _(flusso fratello, sottile)_

1. L'utente fornisce email e password.
2. Il sistema verifica le credenziali.
3. Se corrette, l'utente è autenticato; **altrimenti** (estensione) il sistema rifiuta l'accesso (credenziali non valide).

**Regole di business esercitate.**

- L'**email è univoca** (identità dell'utente).
- Solo dopo registrazione **e** accesso l'utente può usare le funzionalità riservate (`progetto.MD` §Visitatore).

**Requisiti speciali / note implementative.** **Autenticazione "sottile" (D4)**: si crea l'`Utente` con credenziali e si gestisce un accesso minimale; **non** si realizza uno strato di sicurezza completo (hashing forte, sessioni/token, gestione ruoli a runtime), che resta fuori scope (§4.5). Questo UC **rende reale l'"utente corrente"** che le iterazioni 1-3 trattavano come parametro-stub.

**Frequenza d'uso.** Media (registrazione una tantum; accesso frequente).

**Problemi aperti.** Conservazione sicura della password (hashing/salting). Sessioni/token e logout (modellabile come UC separato se richiesto). Conferma email. Recupero password.

**Suggerimento VP.** Collega `Visitatore` a questo UC; modella `Autenticazione` come UC «include» o come flusso fratello. Requirement "Email univoca; accesso richiede registrazione".

---

### UC20 — Consulta Hackathon

| Campo                  | Valore             |
| ---------------------- | ------------------ |
| **Nome**               | Consulta Hackathon |
| **ID**                 | UC20               |
| **Portata**            | HackHub (sistema)  |
| **Livello**            | Obiettivo utente   |
| **Attore primario**    | Visitatore         |
...
**Breve descrizione.** Un **Visitatore** (utente non autenticato) **consulta liberamente** le informazioni pubbliche sugli hackathon: elenco e dettagli pubblici (nome, regolamento, date, luogo, premio). È l'unica funzionalità accessibile senza autenticazione.
...
- **3a. Il Visitatore tenta un'azione riservata** (es. iscrivere un team):
  - 3a.1 Il sistema richiede registrazione e accesso (rimando a UC19).
...
**Suggerimento VP.** Collega `Visitatore` (e, in generalizzazione, gli attori autenticati) a questo UC. Requirement "Visitatore: sola consultazione delle informazioni pubbliche".

---

### UC21 — Aggiungi Mentore
...
ACCESSO (attore Visitatore, finora scollegato)
  UC20 Consulta Hackathon       → informazioni pubbliche (nessuna auth)
  UC19 Registrazione/Autentic.  → Visitatore diventa Utente autenticato  ⇒ abilita TUTTI gli UC riservati (iter. 1-4)
...
STAFF
  UC21 Aggiungi Mentore         → nuovo Incarico (Mentore)  ⇒ abilita UC12/UC13/UC17 per il nuovo Mentore
```

**Dipendenze chiave.** UC16 presuppone UC08 (hackathon concluso con vincitore). UC18 presuppone UC17 (esiste una segnalazione). UC21 alimenta la catena del mentoring (iter. 3) e la governance (UC17). UC19 rende reale l'"utente corrente" assunto come stub in **tutte** le iterazioni precedenti; UC20 è l'unico UC accessibile **senza** autenticazione.

**Continuità coi pattern.** Lo **State** resta sul solo Hackathon (letto, non transito: UC16 richiede "Concluso", UC21 richiede non-"Concluso"). L'**Observer** (iter. 2-3) è **riusato** per le segnalazioni (UC17). L'**Adapter** (iter. 3, Calendar) è **riusato** per il Sistema di Pagamento (UC16): a fine progetto **entrambi** i sistemi esterni di `progetto.MD` sono isolati dalla stessa famiglia di pattern. **Nessun nuovo pattern**: l'ultima iterazione consolida, non aggiunge.
...
| ------------------------------ | --------------------------------------------------- | ---------------------------------------- |
| Membro dello Staff             | consulta l'elenco di tutti gli hackathon            | UC20 (generalizzazione agli autenticati) |
...
| Organizzatore                  | aggiunge Mentori anche dopo la creazione            | **UC21**                                 |
...
| Membro del Team                | consulta gli hackathon                              | UC20                                     |
...
| Utente                         | si registra e accede                                | **UC19**                                 |
| Amministratore Team            | accetta / rifiuta una proposta di call              | UC14 / UC15                              |
| Visitatore                     | consulta le informazioni pubbliche                  | **UC20**                                 |
| Visitatore                     | registrazione e accesso                             | **UC19**                                 |
...
- **Credenziali su `Utente`**: aggiungere `password` (o entità `Credenziale`) per UC19; `RepoUtente.esisteEmail(email)` / `trovaPerEmail(email)`.
...
- **EBC/persistenza** coerenti con `CONVENZIONI_DIAGRAMMI.md`: `Handler*` per-UC (`HandlerPagaMontepremi`, `HandlerSegnalaTeam`, `HandlerGestisciSegnalazione`, `HandlerRegistrazione`, `HandlerConsultaHackathon`, `HandlerAggiungiMentore`), `service` (`GestorePagamento`, `GestoreSegnalazione`, `GestoreAccount`, e riuso `GestoreHackathon` per consultazione/aggiungiMentore), `Repo*` (`RepoPagamento`, `RepoSegnalazione`, riuso `RepoUtente`/`RepoHackathon`). **Nota:** gli `Incarico` (UC21) sono parte dell'aggregato `Hackathon` (creati da `assegnaStaff`/`aggiungiMentore`) e si persistono via `RepoHackathon` — **non** esiste un `RepoIncarico` dedicato.
...
| UC19 | Creator, Controller                                           | crea `Utente`; autenticazione "sottile"                    |
| UC20 | Information Expert, Controller                                | sola lettura; informazioni pubbliche                       |
| UC21 | Creator (Incarico), Controller                                | riuso `assegnaStaff`; nessuna nuova entità                 |

> **Copertura pattern iter. 4:** si **riusano** Adapter (Pagamento) e Observer (segnalazioni); lo **State** è solo letto (pre-condizioni di UC16/UC21). Nessun nuovo pattern — l'ultima iterazione consolida la narrativa architetturale.

### 4.5 Problemi aperti consolidati (decisioni da confermare)

1. **Autenticazione di sicurezza reale**: UC19 è "sottile" (D4); hashing password, sessioni/token, logout, recupero password restano fuori scope. Logout modellabile come UC separato se richiesto.
...
5. **Notifiche Observer opzionali**: pagamento al team vincitore (UC16), incarico al nuovo Mentore (UC21) — consigliate per simmetria, non obbligatorie.
6. **Confine "informazioni pubbliche"** (UC20): quali campi sono pubblici; unificazione con la consultazione dello staff (Membro dello Staff "_consulta l'elenco di tutti gli hackathon_").
